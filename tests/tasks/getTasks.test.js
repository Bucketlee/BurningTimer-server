...
describe("Node Server Request getTasks Function.", () => {
  it("Should answer GET requests for /api/tasks with a 200 status code", async () => {
    // given
    // username = test;
    ...
    // when
    const { body, statusCode } = await request(app).get("/api/tasks").set("authorization", token);
